	...
	if(isset($_SESSION['email'], $_SESSION['phone'])){
		...
		if(isset($_POST['submit'])){
			if(!empty($_FILES['profile_pic']['name'])){
				...
				$profilePic_Name = $_FILES['profile_pic']['name'];
				$profilePic_TmpName = $_FILES['profile_pic']['tmp_name'];
				$profilePic_Size = $_FILES['profile_pic']['size'];
				$profilePic_Error = $_FILES['profile_pic']['error'];
				$profilePic_Type = $_FILES['profile_pic']['type'];

				$profilePic_Ext = explode('.', $profilePic_Name);
				$profilePic_ActualExt = strtolower(end($profilePic_Ext));

				if(in_array($profilePic_ActualExt, $fileAllowed)){
					if($profilePic_Error === 0){
						if($profilePic_Size < 10000000){

							$profilePic_NameNew = "profilepic_".$_SESSION['email'].".".$profilePic_ActualExt;
							$profilePic_Destination = 'uploads/'.$profilePic_NameNew;

							//check if file already exists
							if(file_exists("uploads/".$profilePic_NameNew)){
								//if exist then replace with new one
								// $display_msg = "files there, ";
								if(unlink("uploads/".$profilePic_NameNew)){
									// $display_msg .= "files deleted, ";
									if(move_uploaded_file($profilePic_TmpName, $profilePic_Destination)){
										// $display_msg .= "files moved ";
										$update_stmt = $conn->prepare(
											"UPDATE lawyer SET 
											fullname				=	:fullname, 
											category				=	:category, 
											degree					=	:degree, 
											lawyer_exp			=	:lawyer_exp, 
											address_line_1	=:address_line_1, 
											address_line_2	=	:address_line_2, 
											city						=	:city, 
											zip							=	:zip, 
											profile_pic			=	:profile_pic WHERE 
											email						=	:email"
										);
										$update_rslt = $update_stmt->execute(
											[
												'fullname'				=>	$_POST['fullname'], 
												'category'				=>	$_POST['category'], 
												'degree'					=>	$_POST['degree'], 
												'lawyer_exp'			=>	$_POST['lawyer_exp'], 
												'address_line_1'	=>	$_POST['address1'], 
												'address_line_2'	=>	$_POST['address2'], 
												'city'						=>	$_POST['city'], 
												'zip'							=>	$_POST['zip'], 
												'profile_pic'			=>	$profilePic_Destination,
												'email'						=>	$_SESSION['email']
											]
										);
										if($update_rslt == true){
												$display_msg = "Profile Picture And Info Updated";
										} elseif($update_rslt == false){
											$display_msg = "An error occured while updating Profile Picture and Info";
										}
									}
								}
							} elseif(!file_exists("uploads/".$profilePic_NameNew)){
								// $display_msg = "files therent, ";
								if(move_uploaded_file($profilePic_TmpName, $profilePic_Destination)){
									// $display_msg .= "files moved ";
									$update_stmt = $conn->prepare(
										"UPDATE lawyer SET 
									...
									if($update_rslt == true){
										$display_msg = "Profile Picture Updated ";
									} elseif($update_rslt == false){
										$display_msg = "An error occured while updating Profile Picture";
									}
								}
							}
						}
				...
				if($update_rslt == true){
						$display_msg = "Info Updated ";
				} elseif($update_rslt == false){
					$display_msg = "An error occured while updating Info";
				}
			}
...
<head>
    <meta charset="utf-8">
    <title>Advogate- Lawyer Profile Edit</title>
        ...
        <form action="profile-edit.php" method="post" enctype="multipart/form-data">
            <div class="text-center">
                <div class="figure-profile shadow my-4">
									<figure><img src="<?php echo $row['profile_pic']; ?>" alt=""></figure>
									<div class="btn btn-dark text-white floating-btn">
											<i class="material-icons">camera_alt</i>
											<input type="file" name="profile_pic" class="float-file">
									</div>
                </div>
                <?php
            ...
            <h6 class="subtitle">Basic Information</h6>
                <div class="row">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group float-label active">
                            <input type="email" name="email" class="form-control" id="disabledInput" required="" value="<?php echo $row['email']; ?>" disabled>
                ...
                <div class="row">
                    <div class="col-12 col-md-6">
                        <div class="form-group float-label active mb-0">
                            <input type="tel" name="phone" class="form-control" id="disabledInput" required="" value="<?php echo $row['phone']; ?>" disabled>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group float-label active mb-0">
                            <select name="category" class="form-control">
                                <option value=""<?=$row['category'] == '' ? ' selected' : '';?>>Specialization</option>
                                <option value="Civil Law"<?=$row['category'] == 'Civil Law' ? ' selected' : '';?>>Civil Law</option>
                                <option value="Criminal Law"<?=$row['category'] == 'Criminal Law' ? ' selected' : '';?>>Criminal Law</option>
                                <option value="Corporate Law"<?=$row['category'] == 'Corporate Law' ? ' selected' : '';?>>Corporate Law</option>
                                <option value="Consumer Law"<?=$row['category'] == 'Consumer Law' ? ' selected' : '';?>>Consumer Law</option>
                                <option value="General Law"<?=$row['category'] == 'General Law' ? ' selected' : '';?>>General Law</option>
                                <option value="Real Estate Law"<?=$row['category'] == 'Real Estate Law' ? ' selected' : '';?>>Real Estate Law </option>
                                <option value="Banking Law"<?=$row['category'] == 'Banking Law' ? ' selected' : '';?>>Banking Law</option>
                                <option value="Immigration Law"<?=$row['category'] == 'Immigration Law' ? ' selected' : '';?>>Immigration Law</option>
                            </select>
                            <!-- <input type="text" name="category" class="form-control" required="" value="" > -->
                            <label class="form-control-label">Lawyer Category</label>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12 col-md-6">
                        <div class="form-group float-label active mb-0">
                            <select name="degree" class="form-control">
                                <option value=""<?=$row['category'] == '' ? ' selected' : '';?>>Educational Level</option>
                                <option value="LLB"<?=$row['degree'] == 'LLB' ? ' selected' : '';?>>L.L.B.</option>
                                <option value="LLM"<?=$row['degree'] == 'LLM' ? ' selected' : '';?>>L.L.M.</option>
                            </select>
                            <!-- <input type="text" name="degree" class="form-control" required="" value="" > -->
                            <label class="form-control-label">Educational Degree</label>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12 col-md-6">
                        <div class="form-group float-label active mb-0">
                            <input type="number" name="lawyer_exp" class="form-control" required="" value="<?php echo $row['lawyer_exp']; ?>" >
                            <label class="form-control-label">Lawyer Experience (Years)</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-12 col-md-6">
                        <div class="form-group float-label active">
                            <input type="text" class="form-control" required="" value="<?php echo $row['address_line_1']; ?>" name="address1">
                            <label class="form-control-label">Address line 1</label>
                        </div>
                    </div>
                    <div class="col-12 col-md-6">
                        <div class="form-group float-label active">
                            <input type="text" class="form-control" value="<?php echo $row['address_line_2']; ?>" name="address2">
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="form-group float-label active">
                            <input type="text" class="form-control" required="" value="<?php echo $row['city']; ?>" name="city">
                            <label class="form-control-label">City</label>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="form-group float-label active">
                            <input type="text" class="form-control" required="" value="<?php echo $row['zip']; } ?>" name="zip">
                ...
                <!-- <a href="profile-edit.php" class="btn btn-lg btn-default text-white btn-block btn-rounded shadow"><span>Submit</span></a> -->
                <!-- <div class="row">
                    <div class="col">
                        <a class="btn btn-danger btn-block btn-lg mb-2 btn-rounded" data-toggle="collapse" href="#collapseExample" role="button" aria-expanded="true" aria-controls="collapseExample">CHANGE/UPDATE DOCUMENTS</a>
                        <div class="collapse" id="collapseExample" style="">
                            <div class="card shadow-sm border-0 bg-danger">
                                <div class="card-body">
                                    <div class="alert alert-danger text-center lead" role="alert">
                                        <span class="material-icons" style="font-size: 4rem"> error_outline</span>
                                        <h4>Notice</h4>
                                        <p><b>Old file will be replaced <br>with new file</b></p>
                                    </div>
                                    <?php
                                        foreach($select_rslt as $row){
                                            if(!empty($row['kba_front']) && !empty($row['kba_back']) && (empty($row['sbc_front']) && empty($row['sbc_back']))){
                                                ?>
                                                <div class="form-group">
                                                    <label for="file">KBA Enrollment Card (Front)</label>
                                                    <input type="file" class="form-control" id="file" name="kba_front">
                                                </div>
                                                <div class="form-group">
                                                    <label for="file">KBA Enrollment Card (Back)</label>
                                                    <input type="file" class="form-control" id="file" name="kba_back">
                                                </div>
                                                <?php
                                                ...
                                                <div class="form-group">
                                                    <label for="file">SBC Enrollment Card (Front)</label>
                                                    <input type="file" class="form-control" id="file" name="sbc_front">
                                                </div>
                                                <div class="form-group">
                                                    <label for="file">SBC Enrollment Card (Back)</label>
                                                    <input type="file" class="form-control" id="file" name="sbc_back">
                                                </div>
                                                <?php
                                            }
                                        }
                ...
                <div class="row">
                    <div class="col">
                        <input type="submit" name="submit" class="mt-2 btn btn-lg btn-default btn-block btn-rounded shadow" value="submit">
                    </div>
                </div>
                <br>
            </div>
