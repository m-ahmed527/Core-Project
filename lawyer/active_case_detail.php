<?php

	include 'db.php';
	session_start();

	if(isset($_SESSION['email'], $_SESSION['phone'])){

		if(empty($_GET['ucid'])){
			if(!empty($_POST['ucid'])){
				$ucid = $_POST['ucid'];
			} elseif(empty($_POST['ucid'])){
				header('location: index.php');
			}
		} elseif(!empty($_GET['ucid'])){
			$ucid = $_GET['ucid'];
		}

		// new notification
		$select_new_case_request_stmt = $conn->prepare(
			"SELECT COUNT(*) FROM notification WHERE 
			lawyer_email = :lawyer_email AND notification_status=:notification_status"
		);
		$select_new_case_request_stmt->execute(
			[
				'lawyer_email'				=>	$_SESSION['email'],
				'notification_status'	=>	'user read/lawyer unread'
			]
		);
		$select_new_case_request_rslt = $select_new_case_request_stmt->fetchAll();
		foreach($select_new_case_request_rslt as $unread_case){}

		$select_stmt = $conn->prepare(
			"SELECT * FROM lawyer WHERE 
			email=:email"
		);
		$select_stmt->execute(
			[
				'email'	=>	$_SESSION['email'], 
			]
		);
		$select_rslt = $select_stmt->fetchAll();
		foreach($select_rslt as $row){}
		
		$select_stmt_case = $conn->prepare(
			"SELECT * FROM cases WHERE 
			lawyer_email=:lawyer_email AND
			case_status=:case_status AND
			unique_case_id=:unique_case_id"
		);
		$select_stmt_case->execute(
			[
				'lawyer_email'	    =>	$_SESSION['email'],
				'case_status'		=>	'case active', 
				'unique_case_id'	=>	$ucid
			]
		);
		$select_rslt_case = $select_stmt_case->fetchAll();
		foreach($select_rslt_case as $row1){}

		if(isset($_POST['case_submit'])){
			if(!empty($_POST['ucid']) && !empty($_POST['case_date']) && !empty($_POST['case_detail'])){
				if(!empty($_FILES['case_file']['name'])){

					$file_name = $_FILES['case_file']['name'];
					$file_size = $_FILES['case_file']['size'];
					$file_tmp = $_FILES['case_file']['tmp_name'];
					$file_type = $_FILES['case_file']['type'];
					$file_error = $_FILES['case_file']['error'];

					$file_ext = explode('.', $file_name);
					$file_ext_actual = strtolower(end($file_ext));
					
					$allowed_ext = array('jpeg', 'jpg', 'png');

					if(in_array($file_ext_actual, $allowed_ext)){
						if($file_error === 0){
							$file_new_name = "case_file_".$_SESSION['email'].".".$file_ext_actual;
							$file_destination = 'uploads/'.$file_new_name;

							if(move_uploaded_file($file_tmp, $file_destination)){
								$insert_case_track_stmt = $conn->prepare(
									"INSERT INTO track_cases (
										case_id, 
										client_email, 
										lawyer_email, 
										case_category, 
										case_detail, 
										case_date,
										case_file
									) VALUES (
										:case_id, 
										:client_email, 
										:lawyer_email, 
										:case_category, 
										:case_detail, 
										:case_date,
										:case_file
									)"
								);
								$insert_case_track_stmt->execute(
									[
										'case_id'				=>	$_POST['ucid'], 
										'client_email'	=>	$row1['client_email'], 
										'lawyer_email'	=>	$row1['lawyer_email'], 
										'case_category'	=>	$row1['case_category'], 
										'case_detail'		=>	$_POST['case_detail'], 
										'case_date'			=>	$_POST['case_date'],
										'case_file'			=>	$file_destination
									]
								);
								if($insert_case_track_stmt == true){
									$update_case_not_status = $conn->prepare(
										"INSERT INTO notification (
											case_id,
											client_email, 
											lawyer_email, 
											case_status,
											notification_status,
											case_category,
											case_date
										) VALUES (
											:case_id,
											:client_email, 
											:lawyer_email, 
											:case_status,
											:notification_status,
											:case_category,
											:case_date
										)"
									);
									$update_case_not_status->execute(
										[
											'case_id'							=>	$ucid,
											'client_email'				=>	$row1['client_email'],
											'lawyer_email'				=>	$_SESSION['email'],
											'case_status'   			=>	'case updated',
											'notification_status'	=>	'user unread/lawyer read',
											'case_category'				=>	$row1['case_category'],
											'case_date'						=>	$_POST['case_date']
										]
									);
									$display_msg = "Case updated with file";
								} elseif($insert_case_track_stmt == false){
									$display_msg = "Case could not be updated with file";
								}
							}
						}
					}
				} elseif(empty($_FILES['case_file']['name'])){
					$insert_case_track_stmt = $conn->prepare(
						"INSERT INTO track_cases (
							case_id, 
							client_email, 
							lawyer_email, 
							case_category, 
							case_detail, 
							case_date
						) VALUES (
							:case_id, 
							:client_email, 
							:lawyer_email, 
							:case_category, 
							:case_detail, 
							:case_date
						)"
					);
					$insert_case_track_stmt->execute(
						[
							'case_id'				=>$_POST['ucid'], 
							'client_email'	=>$row1['client_email'], 
							'lawyer_email'	=>$row1['lawyer_email'], 
							'case_category'	=>$row1['case_category'], 
							'case_detail'		=>$_POST['case_detail'], 
							'case_date'			=>$_POST['case_date']
						]
					);
					if($insert_case_track_stmt == true){
						$update_case_not_status = $conn->prepare(
							"INSERT INTO notification (
								case_id,
								client_email, 
								lawyer_email, 
								case_status,
								notification_status,
								case_category,
								case_date
							) VALUES (
								:case_id,
								:client_email, 
								:lawyer_email, 
								:case_status,
								:notification_status,
								:case_category,
								:case_date
							)"
						);
						$update_case_not_status->execute(
							[
								'case_id'							=>	$ucid,
								'client_email'				=>	$row1['client_email'],
								'lawyer_email'				=>	$_SESSION['email'],
								'case_status'   			=>	'case updated',
								'notification_status'	=>	'user unread/lawyer read',
								'case_category'				=>	$row1['case_category'],
								'case_date'						=>	$_POST['case_date']
							]
						);
						$display_msg = "Case updated";
					} elseif($insert_case_track_stmt == false){
						$display_msg = "Case could not be updated";
					}		
				}
			}
		}

		$select_case_tracking_stmt = $conn->prepare(
			"SELECT * FROM track_cases WHERE
			client_email=:client_email AND
			lawyer_email=:lawyer_email AND 
			case_category=:case_category AND
			case_id=:case_id"
		);
		$select_case_tracking_stmt->execute(
			[
				'client_email'	=> $row1['client_email'], 
				'lawyer_email'	=> $row1['lawyer_email'], 
				'case_category'	=> $row1['case_category'], 
				'case_id'				=> $row1['unique_case_id']
			]
		);
		$select_case_tracking_rslt = $select_case_tracking_stmt->fetchAll();
		foreach($select_case_tracking_rslt as $row2){}

		if(isset($_POST['case_finalize'])){
			$update_case_finalize_stmt = $conn->prepare(
				"UPDATE cases SET 
				case_status=:case_status WHERE
				client_email=:client_email AND
				lawyer_email=:lawyer_email AND
				case_category=:case_category AND
				unique_case_id=:unique_case_id"
			);
			$update_case_finalize_stmt->execute(
				[
					'case_status'			=>	'case resolved', 
					'client_email'		=>	$row2['client_email'], 
					'lawyer_email'		=>	$row2['lawyer_email'], 
					'case_category'		=>	$row2['case_category'], 
					'unique_case_id'	=>	$row2['case_id']
				]
			);
			if($update_case_finalize_stmt == true){
				$update_case_not_status = $conn->prepare(
					"INSERT INTO track_cases (
						case_id,
						client_email, 
						lawyer_email, 
						case_date, 
						case_status,
						case_category
					) VALUES (
						:case_id,
						:client_email, 
						:lawyer_email, 
						:case_date, 
						:case_status,
						:case_category
					)"
				);
				$update_case_not_status->execute(
					[
						'case_id'					=>	$ucid,
						'client_email'				=>	$row1['client_email'],
						'lawyer_email'				=>	$_SESSION['email'],
						'case_date'					=>	date('d/m/y').date('H:m:i'), 
						'case_status'   			=>	'case resolved',
						'case_category'				=>	$row1['case_category']
					]
				);

				$update_case_not_status = $conn->prepare(
					"INSERT INTO notification (
						case_id,
						client_email, 
						lawyer_email, 
						case_status,
						notification_status,
						case_category,
						case_date
					) VALUES (
						:case_id,
						:client_email, 
						:lawyer_email, 
						:case_status,
						:notification_status,
						:case_category,
						:case_date
					)"
				);
				$update_case_not_status->execute(
					[
						'case_id'							=>	$ucid,
						'client_email'				=>	$row1['client_email'],
						'lawyer_email'				=>	$_SESSION['email'],
						'case_status'   			=>	'case resolved',
						'notification_status'	    =>	'user unread/lawyer read',
						'case_category'				=>	$row1['case_category'],
						'case_date'					=>	date('d/m/y').date('H:m:i')
					]
				);
				// $update_case_track_stmt = $conn->prepare(
				// 	"UPDATE track_cases SET 
				// 	case_status=:case_status WHERE
				// 	client_email=:client_email AND
				// 	lawyer_email=:lawyer_email AND
				// 	case_category=:case_category"
				// );
				// $update_case_track_stmt->execute(
				// 	[
				// 		'case_status'		=>	'resolved', 
				// 		'client_email'	=>	$row1['client_email'], 
				// 		'lawyer_email'	=>	$row1['lawyer_email'], 
				// 		'case_category'	=>	$row1['case_category']
				// 	]
				// );
				// if($update_case_track_stmt == true){
					$display_msg = "Case status updated";
					header('location: index.php');
				// } elseif($update_case_track_stmt == false){
				// 	$display_msg = "Case could not be updated";
				// }
			} elseif($update_case_finalize_stmt == false){
				$display_msg = "Case could not be updated";
			}
		}

?>
<!DOCTYPE html>
<html lang="zxx">

<head>
    <meta charset="utf-8">
    <title>Advogate- Advocate Active Case Detail</title>
    <meta content="width=device-width, initial-scale=1.0" name="viewport">
    <meta content="Advogate aims to bridge the gap between advocates and clients. It allows user to find the best available lawyer as per thier required case in super affordable price" name="description">
    <meta content="" name="keywords">
    <meta content="" name="author">
    <!--[if lt IE 9]>
    <script src="js/html5shiv.js"></script>
    <![endif]-->
    <!-- CSS Files
    ================================================== -->
    <link id="bootstrap" href="css/bootstrap.min.css" rel="stylesheet" type="text/css" />
    <link id="bootstrap-grid" href="css/bootstrap-grid.min.css" rel="stylesheet" type="text/css" />
    <link id="bootstrap-reboot" href="css/bootstrap-reboot.min.css" rel="stylesheet" type="text/css" />
    <link href="css/animate.css" rel="stylesheet" type="text/css">
    <link href="css/owl.carousel.css" rel="stylesheet" type="text/css">
    <link href="css/owl.theme.css" rel="stylesheet" type="text/css">
    <link href="css/owl.transitions.css" rel="stylesheet" type="text/css">
    <link href="css/magnific-popup.css" rel="stylesheet" type="text/css">
    <link href="css/jquery.countdown.css" rel="stylesheet" type="text/css">
    <link href="css/style.css" rel="stylesheet" type="text/css">
    <!-- color scheme -->
    <link id="colors" href="css/colors/scheme-01.css" rel="stylesheet" type="text/css">
		<link href="css/coloring.css" rel="stylesheet" type="text/css">
		<link rel="stylesheet" href="vendor/materializeicon/material-icons.css">
		<link href="https://fonts.googleapis.com/css?family=Roboto:300,400,500,700&amp;display=swap" rel="stylesheet">
		<link href="vendor/bootstrap-4.4.1/css/bootstrap.min.css" rel="stylesheet">
		<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
		<link href="vendor/swiper/css/swiper.min.css" rel="stylesheet">
</head>

<body>
    <div id="wrapper">
        <div id="topbar" class="text-white bg-color">
            <div class="container">
                <div class="topbar-left sm-hide">
                    <span class="topbar-widget tb-social">
                        <a href="#"><i class="fa fa-facebook"></i></a>
                        <a href="#"><i class="fa fa-twitter"></i></a>
                        <a href="#"><i class="fa fa-instagram"></i></a>
                    </span>
                </div>
                <div class="topbar-right">
                    <div class="topbar-right">
                        <span class="topbar-widget"><a href="#">Privacy policy</a></span>
                        <span class="topbar-widget"><a href="#">Request Quote</a></span>
                        <span class="topbar-widget"><a href="#">FAQ</a></span>
                        <span class="topbar-widget"><a href="log-out.php">Logout</a></span>
                    </div>
                </div>
                <div class="clearfix"></div>
            </div>
        </div>
        <!-- header begin -->
        <header class="transparent">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="de-flex sm-pt10">
                            <div class="de-flex-col">
                                <!-- logo begin -->
                                <div id="logo">
                                    <a href="indexdata.php">
                                        <img alt="" class="logo" src="../images/logo1-light1.png" />
                                        <img alt="" class="logo-2" src="../images/logo1.png" />
                                    </a>
                                </div>
                                <!-- logo close -->
                            </div>
                            <div class="de-flex-col header-col-mid">
                                <!-- mainmenu begin -->
                                <ul id="mainmenu">
                                    <li><a href="indexdata.php">Home</a></li>
                                    <li><a href="notification.php">Notifications</a></li>
                                    <li><a href="transactions.php">Transactions</a></li>
                                    <li><a href="profile.php">Profile</a></li>
                                    <!-- <li><a href="about.php">About</a>
                                        <ul>
                                            <li><a href="about.php">About Us</a></li>
                                            <li><a href="team.php">The Team</a></li>
                                        </ul>
                                    </li>
                                    <li><a href="practice-areas.php">Practice Areas</a></li>
                                    <li><a href="news.php">News</a></li>
                                    <li><a href="contact.php">Contact</a></li> -->
                                </ul>
                                <!-- mainmenu close -->
                            </div>
                            <div class="de-flex-col">
                                <div class="h-phone md-hide"><span>Need&nbsp;Help?</span><i class="fa fa-phone"></i>92 21 35220318</div>
                                <span id="menu-btn"></span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </header>
        <!-- header close -->
        <!-- content begin -->
        <div class="no-bottom no-top" id="content">
            <div id="top"></div>
            <!-- section begin -->
            <section id="subheader" class="text-light" data-stellar-background-ratio=".2" data-bgimage="url(../images/background/subheader1.jpg) top">
                <div class="center-y relative text-center">
                    <div class="container">
                        <div class="row">
                            <div class="col text-center">
                                <div class="spacer-single"></div>
                                <h1>Case tracking</h1>
                                <!-- <p>Reputation. Respect. Result.</p> -->
                            </div>
                            <div class="clearfix"></div>
                        </div>
                    </div>
                </div>
            </section>
            <!-- section close -->
            <section>
							<div class="container">
								<div class="card bg-template shadow mt-4 h-100">
									<div class="card-body">
										<div class="row">
											<div class="col-auto">
												<figure class="avatar avatar-60"><img src="../images/user1.png" alt=""></figure>
											</div>
											<div class="col pl-0 align-self-center">
												<h5 class="mb-1"><!-- Ammy Jahnson --><?php echo $row['fullname']; ?></h5>
												<!-- <p class="text-mute small">Work, London, UK</p> -->
											</div>
										</div>
									</div>
								</div>
							</div>
							<div class="container">
								<!-- page content here -->
								<h6 class="subtitle">Case Category</h6>
								<div class="row">
									<div class="col-12 px-0">
										<ul class="list-group list-group-flush border-top border-bottom">
											<?php
												$day_page = "'active_case_detail_day.php'";
												foreach($select_case_tracking_rslt as $row_case_track){ 
													if(empty($row_case_track['case_file'])){
														echo '<li class="list-group-item">
																		<div class="row align-items-center" onclick="location.href=\''.htmlspecialchars('active_case_detail_day.php?ucid='.urlencode($row_case_track['case_id']).'&date='.urlencode($row_case_track['case_date'])).'\'">
																			<div class="col align-self-center pr-0">
																				<p class="font-weight-bold mb-1">Date/Time </p>
																				<h6 class="font-weight-normal mb-1">'.$row_case_track['case_date'].'</h6>
																			</div>
																			<div class="col-auto mt-1 ">
																				<p class="font-weight-bold mb-1 text-center">Details </p>
																				<h6 class="font-weight-normal" style="word-wrap: break-word">'.$row_case_track['case_detail'].'</h6>
																			</div>
																		</div>
																	</li>';
													} elseif(!empty($row_case_track['case_file'])){
															echo '<li class="list-group-item">
																			<div class="row align-items-center" onclick="location.href=\''.htmlspecialchars('active_case_detail_day.php?ucid='.urlencode($row_case_track['case_id']).'&date='.urlencode($row_case_track['case_date'])).'\'">
																				<div class="col align-self-center pr-0">
																					<p class="font-weight-bold mb-1 ">Date/Time </p>
																					<h6 class="font-weight-normal mb-1">'.$row_case_track['case_date'].'</h6>
																				</div>
																				<div class="col-auto mt-1 " >
																				<p class="font-weight-bold mb-1 text-center">Details </p>
																					<h6 class="font-weight-normal" style="word-wrap: break-word">'.$row_case_track['case_detail'].'</h6>
																				</div>
																				<div class="col-auto mt-1">
																				<p class="font-weight-bold mb-1 text-center">File </p>
																					<a href="'.$row_case_track['case_file'].'" target="_blank">
																						<button type="button" class="mb-2 btn btn-default btn-rounded-54" ><i class="material-icons">assignment</i></button>
																					</a>
																				</div>
																			</div>
																		</li>';
													}
												}
											?>
										</ul>
									</div>
								</div>
								<div class="row">
									<div class="col mt-4" style="text-align-last:center;">
										<button type="button" class="btn btn-default rounded-0" data-toggle="modal" data-target="#casemodal">Add Details</button>
										<form action="active_case_detail.php" method="post" style="display: inline;">
											<input type="hidden" name="ucid" class="btn btn-lg btn-default rounded-0" value="<?php echo $ucid; ?>">
											<input type="submit" name="case_finalize" class="btn btn-primary rounded-0" value="Finalize Case">
										</form>
										<br>
										<br>
										<?php
											if(isset($display_msg)){
												echo $display_msg;
											}
										?>
									</div>
								</div>
								<!-- page content ends -->
							</div>
            </section>
        </div>
        <!-- content close -->
        <!-- <section class="pt40 pb40 bg-color text-light">
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-md-8 mb-sm-30 text-lg-left text-sm-center">
                            <h3 class="no-bottom">Hire Your Law Specialist Now!</h3>
                        </div>
                        <div class="col-md-4 text-lg-right rtl-lg-left text-sm-center">
                            <a href="http://localhost/advodesktop/user/signup.php" class="btn-custom btn-black light">Sign Up Now</a>
                        </div>
                    </div>
                </div>
            </section> -->
        <a href="#" id="back-to-top"></a>
        <!-- footer begin -->
        <footer>
            <<div class="container">
                <div class="row">
                    <div class="col-lg-4">
                        <div class="widget">
                            <a href="index.php"><img alt="" class="img-fluid mb20" src="../images/logo1-light1.png"></a>
                            <address class="s1">
                                <span><i class="id-color fa fa-map-marker fa-lg"></i>Kashif Center, Near Hotel Mehran, Main Shahrah-e-Faisal, Karachi</span>
                                <span><i class="id-color fa fa-phone fa-lg"></i>+92 21 35220318</span>
                                <span><i class="id-color fa fa-envelope-o fa-lg"></i><a href="mailto:contact@example.com">support@advogate.com.pk</a></span>
                            </address>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <h5 class="id-color mb20">Practice Areas</h5>
                        <ul class="ul-style-2">
                            <li>Corporate and M&A</li>
                            <li>Construction and Real Estate</li>
                            <li>Commercial Duspute Resolution</li>
                            <li>Employment</li>
                            <li>Banking and Finance</li>
                        </ul>
                    </div>
                    <div class="col-lg-4">
                        <div class="widget">
                            <h5 class="id-color">Newsletter</h5>
                            <p>Signup for our newsletter to get the latest news, updates and special offers in your inbox.</p>
                            <form action="blank.php" class="row" id="form_subscribe" method="post" name="form_subscribe">
                                <div class="col text-center">
                                    <input class="form-control" id="name_1" name="name_1" placeholder="enter your email" type="text" /> <a href="#" id="btn-submit"><i class="fa fa-long-arrow-right"></i></a>
                                    <div class="clearfix"></div>
                                </div>
                            </form>
                            <div class="spacer-10"></div>
                            <small>Your email is safe with us. We don't spam.</small>
                        </div>
                    </div>
                </div>
            </div>
            <div class="subfooter">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="de-flex">
                                <div class="de-flex-col">
                                    &copy; Copyright 2020 - Justica by Designesia
                                </div>
                                <div class="de-flex-col">
                                    <div class="social-icons">
                                        <a href="#"><i class="fa fa-facebook fa-lg"></i></a>
                                        <a href="#"><i class="fa fa-twitter fa-lg"></i></a>
                                        <a href="#"><i class="fa fa-linkedin fa-lg"></i></a>
                                        <a href="#"><i class="fa fa-pinterest fa-lg"></i></a>
                                        <a href="#"><i class="fa fa-rss fa-lg"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
				<!-- footer close -->
				<div class="modal fade" id="casemodal" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header border-0">
            <h5> Case Details </h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
            </button>
          </div>
          <form action="active_case_detail.php" method="post" enctype="multipart/form-data">
            <div class="modal-body pt-0">
							<input type="hidden" name="case_date" value="<?php echo date('d/m/y'); echo " | "; echo date('H:i:s'); ?>">
							<input type="hidden" name="ucid" value="<?php echo $row1['unique_case_id']; ?>">
              <div class="form-group mt-2">
                <label for="">Details</label>
                <textarea name="case_detail" class="form-control form-control-lg" rows="5"></textarea>
              </div>
							<div class="form-group mt-2">
								<label for="">File (if any)</label>
								<input type="file" class="form-control" name="case_file" id="file">
							</div>
              <!-- <div class="form-group mt-2">
								<label for="file">Documents</label>
								<input type="file" name="case_file" class="form-control" id="file">
              </div> -->
            </div>
            <div class="modal-footer border-0">
              <!-- <button type="button" name="" class="btn btn-default btn-lg btn-rounded shadow btn-block" class="close" data-dismiss="modal">Done</button> -->
              <input type="submit" name="case_submit" class="btn btn-default btn-lg btn-rounded shadow btn-block" value="Done">
            </div>
          </form>
        </div>
      </div>
    </div>
        <div id="preloader">
            <div class="spinner">
                <div class="bounce1"></div>
                <div class="bounce2"></div>
                <div class="bounce3"></div>
            </div>
        </div>
    </div>
    <!-- Javascript Files
    ================================================== -->
    <script src="js/jquery.min.js"></script>
    <script src="js/bootstrap.min.js"></script>
    <script src="js/wow.min.js"></script>
    <script src="js/jquery.isotope.min.js"></script>
    <script src="js/easing.js"></script>
    <script src="js/owl.carousel.js"></script>
    <script src="js/validation.js"></script>
    <script src="js/jquery.magnific-popup.min.js"></script>
    <script src="js/enquire.min.js"></script>
    <script src="js/jquery.stellar.min.js"></script>
    <script src="js/jquery.plugin.js"></script>
    <script src="js/typed.js"></script>
    <script src="js/jquery.countTo.js"></script>
    <script src="js/jquery.countdown.js"></script>
    <script src="js/designesia.js"></script>
</body>

</html>
<?php
	} else{
			header('location: log-out.php');
	}
?>