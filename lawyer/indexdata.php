<?php
		...
		if(isset($_SESSION['email'], $_SESSION['phone'])){
			...
			// Responding to case request
		if((!empty($_POST['unique_case_id']) && !empty($_POST['client_email']) && !empty($_POST['case_category']) && !empty($_POST['case_date'])  && !empty($_POST['id'])) && (!empty($_POST['submit_accept']) || !empty($_POST['submit_decline']))){
			if(!empty($_POST['submit_accept'])){

				$update_case_status_stmt = $conn->prepare(
					"UPDATE cases SET 
					case_status=:case_status WHERE 
					unique_case_id=:unique_case_id AND 
					client_email=:client_email AND 
					case_category=:case_category AND 
					lawyer_email=:lawyer_email"
				);
				$update_case_status_rslt = $update_case_status_stmt->execute(
					[
						'case_status'					=>	'case active', 
						'unique_case_id'			=>	$_POST['unique_case_id'],
				...
				if($update_case_status_rslt == true){

					$select_relative_case_id_stmt = $conn->prepare(
					...
					$select_relative_case_id_stmt->execute(
						[
							'unique_case_id'=>	$_POST['unique_case_id'], 
							'case_status'		=>	'case active',
							'client_email'	=>	$_POST['client_email'],
					...
					$select_relative_case_id_rslt = $select_relative_case_id_stmt->fetchAll();
					foreach($select_relative_case_id_rslt as $row){}

					$update_notification_stmt = $conn->prepare(
						"UPDATE notification SET notification_status=:notification_status WHERE id=:id"
					);
					$update_notification_rslt = $update_notification_stmt->execute(
						[
					...
					$insert_case_tracking_stmt = $conn->prepare(
					...
					$insert_case_tracking_rslt = $insert_case_tracking_stmt->execute(
						[
							'case_id'				=>	$_POST['unique_case_id'], 
							'client_email'	=>	$_POST['client_email'],
							'lawyer_email'	=>	$_SESSION['email'],
							'case_category'	=>	$_POST['case_category'],
							'case_date'			=>	$_POST['case_date'],
					...
					$insert_into_notification_stmt = $conn->prepare(
					...
					$insert_into_notification_rslt = $insert_into_notification_stmt->execute(
						[
							'case_id' 						=> $_POST['unique_case_id'], 
							'client_email'				=> $_POST['client_email'], 
							'lawyer_email' 				=> $_SESSION['email'], 
					...
					if($insert_case_tracking_rslt == true && $insert_into_notification_rslt == true){
						$display_msg = "You have accepted the case request.";
					} elseif($insert_case_tracking_stmt == false || $insert_into_notification_rslt == false){
						$display_msg = "Request was accepted but there was something wrong while responding to request.";
					}
				} elseif($update_case_status_rslt == false){
					$display_msg = "A problem occured while responding to client's request.";
				}
			} elseif(!empty($_POST['submit_decline'])){
				$update_case_status_stmt = $conn->prepare(
					"UPDATE cases SET  
					case_status=:case_status WHERE 
					client_email=:client_email AND 
				...
				if($update_case_status_rslt == true){
					...
					if($insert_case_tracking_rslt == true && $insert_into_notification_rslt == true){
						$display_msg = "You have declined the case request.";
					} elseif($insert_case_tracking_rslt == false || $insert_into_notification_rslt == false){
						$display_msg = "Request was accepted but there was something wrong while responding to request.";
					}
				} elseif($update_case_status_rslt == false){
					$display_msg = "A problem occured while responding to client's request.";
...
<head>
    <meta charset="utf-8">
    <title>Adovogate- Advocates Home</title>
    <meta content="width=device-width, initial-scale=1.0" name="viewport">
    <meta content="Advogate aims to bridge the gap between advocates and clients. It allows user to find the best available lawyer as per thier required case in super affordable price
" name="description">
														...
														<div class="card-body">
																<div class="row">
																		...
																		<div class="col pl-0 align-self-center">
																				<h5 class="mb-1"><!-- Ammy Jahnson --><?php echo $row_user['fullname']; ?>
																					<small class="float-right mt-1 ml-1">
																						<?php 
																							if($row_user['is_authentic'] == 0){ 
																								echo "Not Authentic"; 
																							} elseif($row_user['is_authentic'] == 1){ 
																								echo "Authentic"; 
																							} 
																						?>
																					</small>
																				</h5>
																				<p class="text-mute small"><?php echo $row_user['category']; ?>yer</p>
																		</div>
																</div>
														</div>
												</div>
															...
															<div class="row">
																	<div class="col align-self-center">
																			<div class="col" onclick="location.href='active_case_page.php'">
																					<h5 class="mb-0 font-weight-normal">Active Cases (
																						<?php
